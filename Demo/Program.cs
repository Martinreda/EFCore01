...
//In EF Core: There is no direct support for Right Outer Join in LINQ.

//To achieve the same result, you can use a Left Join but swap the tables.
#endregion

#region Cross join 
/*In SQL: A Cross Join returns the Cartesian product of two tables.

Example: If Table A has 5 rows and Table B has 3 rows → result will have 15 rows.

In EF Core (LINQ): You can achieve this by using two from clauses without a join condition.

This generates all possible combinations between the two tables.*/
#endregion
